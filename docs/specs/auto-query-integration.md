...
2. **OpenCode + Claude Code current state**: the MCP server exposes 37 tools (`mempalace_search`, `mempalace_kg_query`, `mempalace_diary_read`, `mempalace_list_wings`, `mempalace_get_taxonomy`, `mempalace_traverse`, `mempalace_status`, `mempalace_kg_timeline`, etc.). Per `docs/integrations/opencode.md` §"A note on automatic context injection", today's recall path is *agents explicitly invoking the MCP tools*. There is no per-turn classifier deciding when memory would help; the agent decides, often does not, and the user has to prompt "check mempalace for X" to make it happen.
...
The classifier uses 5 of 37 tools. The rest are management/write surfaces that auto-query has no business touching.
