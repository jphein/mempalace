...
**Current integration shape (JP's fork, local-only):** repo cloned at `~/Projects/palace-daemon`; daemon to run as systemd `--user` service pointing at `~/Projects/mempalace-data/palace/`; Claude Code MCP config rewired from direct `mempalace-mcp` stdio to daemon's `mempalace-mcp.py` HTTP client; `.claude-plugin/hooks/mempal-{stop,precompact}-hook.sh` swapped for `clients/hook.py`. Not changing the plugin marketplace default yet — this is JP's personal-install configuration while we validate the swap. If it holds for ~a week, we'll evaluate shipping an opt-in daemon-mode in the marketplace plugin.

**Out-of-scope for now:** Postgres + pgvector. Would solve everything palace-daemon does plus native multi-machine, but requires writing a new `BaseBackend` implementation and migrating 135K+ drawers off ChromaDB. bensig's upcoming TypeScript rewrite (announced in Discord) will pick its own storage layer, and building a Postgres backend now risks sunk cost if TS chooses something different. Re-evaluate once the TS spec is public.
