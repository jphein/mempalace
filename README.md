...
[![version-shield](https://img.shields.io/badge/version-3.3.3-4dc9f6?style=flat-square&labelColor=0a0e14)](https://github.com/MemPalace/mempalace/releases)
...
### Multi-client coordination — flock interim fix landed; palace-daemon integration planned

Several users have hit the "multiple clients hammering one palace" pattern — @worktarget's #904 report, the ChromaDB concurrency family in #357 / #521 / #832, and the multi-machine case (laptop → home server palace).

**Interim fix (v3.3.4):** Claude Code spawns one `mcp_server.py` process per open terminal, and stop hooks spawn additional short-lived processes on every fire — all opening independent `PersistentClient` instances against the same palace directory. ChromaDB has no inter-process write locking; concurrent `col.add/upsert/update/delete` from N processes corrupts the HNSW segment, causing the next read to SIGSEGV in `chromadb_rust_bindings`. We landed `_palace_write_lock()` — a `fcntl.flock(LOCK_EX)` on `$palace/.write.lock` — wrapping all four write paths in `mcp_server.py`. Serializes cross-process writes on the same machine; `flock` auto-releases on process death so a crash can't deadlock.

**Full fix (planned):** @rboarescu's [palace-daemon](https://github.com/rboarescu/palace-daemon) is a FastAPI gateway with three asyncio semaphores (read / write / mine) and a dedicated mine lane so bulk imports never starve interactive queries. Also ships an MCP-over-HTTP proxy for remote machines. Its design note pins correctness floor at MemPalace ≥3.3.2, which matches our internal-reliability stack — the two are compositional, not competitive. We're pulling palace-daemon in as a dependency: the fork will optionally route all palace I/O through the daemon, fixing the same-machine case properly and enabling the multi-machine case the flock approach can't address.
