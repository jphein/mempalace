...
15. ~~**fix: `_get_client()` get-then-create guard**~~ — **NOT currently in fork main; #1262 (Legion345) in flight to land it upstream**. Guard landed in fork as `d3a2d22` on 2026-04-18 (`ChromaBackend.get_collection` + `mcp_server._get_collection`), then was silently dropped during the develop merge-conflict resolution in `7e18a70` on 2026-04-25. Tracing the call graph on current fork main: chromadb's `get_or_create_collection` is reached from two direct sites (`backends/chroma.py:1061`, `mcp_server.py:295`), both passing identical metadata (`hnsw:space`, `hnsw:num_threads: 1`, `_HNSW_BLOAT_GUARD`). The fork's 2-arg `ChromaBackend.get_or_create_collection(path, name)` legacy shim at `chroma.py:1101` (used by `migrate.py:237` and `cli.py:1002`) delegates through `get_collection(create=True)` to the same metadata-passing line — so metadata is consistent across all opens. The residual SIGSEGV exposure is legacy palaces created under older chromadb whose *stored* metadata differs from the current expanded set, not a structural call-graph divergence. Issue #1089 was filed 2026-04-21 documenting the pattern; #1262 (filed 2026-04-28 by @Legion345) implements `try get_collection / except chromadb.errors.NotFoundError → create_collection` — catching the renamed-in-1.5.x exception, an improvement over our older `InvalidCollectionException` reference. Once #1262 merges, our row clears via develop sync. **Two prior 2026-04-30 wordings (393ac89, 260e2a2) got the metadata story wrong in different directions — third try here matches the actual call-graph trace.**
