...
15. ~~**fix: `_get_client()` get-then-create guard**~~ — **NOT currently in fork main; #1262 (Legion345) in flight to land it upstream**. Guard landed in fork as `d3a2d22` on 2026-04-18 (`ChromaBackend.get_collection` + `mcp_server._get_collection`), then was silently dropped during the develop merge-conflict resolution in `7e18a70` on 2026-04-25. Current fork main calls `get_or_create_collection` with stable metadata (`hnsw:space`, `hnsw:num_threads: 1`, `_HNSW_BLOAT_GUARD`) — the *consistency* of metadata across opens sidesteps the ChromaDB 1.5.x metadata-mismatch SIGSEGV without actually carrying the defensive guard. Issue #1089 was filed 2026-04-21 documenting the pattern; #1262 (filed 2026-04-28 by @Legion345) implements it as `try get_collection / except chromadb.errors.NotFoundError → create_collection` (catching the renamed-in-1.5.x exception, which is an improvement over our older `InvalidCollectionException` reference). Once #1262 merges, our row clears via develop sync; until then, the fork is exposed if upstream ever changes default `hnsw:*` metadata between releases (the same way our own metadata drift caused the original crash).
