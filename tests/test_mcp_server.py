...
class TestCheckpointRouting:
    """Phase B — Stop-hook checkpoint diary entries route to the
    dedicated ``mempalace_session_recovery`` collection rather than the
    searchable ``mempalace_drawers`` collection. See
    ...
    def test_diary_write_routes_checkpoint_to_recovery(self, monkeypatch, config, palace_path, kg):
        ...
        from mempalace.palace import (
            get_collection as _get_main,
            get_session_recovery_collection,
        )

        result = tool_diary_write(
            agent_name="hooks",
            entry="CHECKPOINT: 2026-04-25 18:00 — session ended",
            topic="checkpoint",
        )
        assert result["success"] is True

        main = _get_main(palace_path, create=True)
    ...
    def test_diary_write_routes_general_to_main(self, monkeypatch, config, palace_path, kg):
        ...
        tool_diary_write(
            agent_name="TestAgent",
            entry="A regular journal entry about today's work.",
            topic="musings",
        )
    ...
    def test_search_does_not_see_new_checkpoints(self, monkeypatch, config, palace_path, kg):
        ...
        unique = "engram-quasar-fluxor"
        tool_diary_write(
            agent_name="hooks",
            entry=f"CHECKPOINT: 2026-04-25 — {unique}",
            topic="checkpoint",
        )
        ...
        assert all("CHECKPOINT:" not in (h.get("content") or "") for h in hits)


class TestSessionRecoveryRead:
    """Phase C — ``tool_session_recovery_read`` MCP handler reads from the
    recovery collection only, with filters for session_id, agent,
    since/until, wing, and limit. See
    ``docs/superpowers/specs/2026-04-25-checkpoint-collection-split.md``.
    """
    ...
    def test_filters_by_agent(self, monkeypatch, config, palace_path, kg):
        ...
        tool_diary_write(agent_name="hooks", entry="from hooks", topic="checkpoint")
        tool_diary_write(agent_name="other", entry="from other", topic="checkpoint")

        result = tool_session_recovery_read(agent="hooks")
        assert result["total"] == 1
        assert result["entries"][0]["content"] == "from hooks"

    def test_respects_limit(self, monkeypatch, config, palace_path, kg):
        _patch_mcp_server(monkeypatch, config, kg)
        _client, _col = _get_collection(palace_path, create=True)
        del _client
        from mempalace.mcp_server import tool_diary_write, tool_session_recovery_read
    ...
    def test_handles_none_metadata(self, monkeypatch, config, palace_path, kg):
        """Defensive: tolerates ``None`` entries in the metadatas list at
        read time (legacy / partial-write data shape; ChromaDB emits this
        even though it rejects writing it). Mirrors the #999 / #1094 /
        #1201 family of None-metadata guards."""
        _patch_mcp_server(monkeypatch, config, kg)
        ...
        class _MockCol:
            def get(self, **_kwargs):
                return {
                    "ids": ["legacy_1"],
                    "documents": ["legacy entry"],
                    "metadatas": [None],  # the failure case
                }

        monkeypatch.setattr(
            mcp_server,
            "_get_session_recovery_collection",
            lambda create=False: _MockCol(),
        )

        result = mcp_server.tool_session_recovery_read()
        assert isinstance(result["entries"], list)
        assert result["total"] == 1
        # Entry survives with empty metadata defaults.
        assert result["entries"][0]["agent"] == ""
        assert result["entries"][0]["session_id"] == ""
