...
def test_sqlite_integrity_errors_reports_a_path_python_cannot_encode(tmp_path):
    ...
    # Feature-detect rather than version-gate: the percent-encoding behavior
    # that first shipped in 3.13 was backported to 3.12 point releases
    # (observed on 3.12.13), so ``sys.version_info < (3, 13)`` misfires there.
    # What the invariant needs is only "when pathname2url raises on this
    # interpreter, the probe reports it instead of raising".
    try:
        pathname2url_raises = False
        urllib.request.pathname2url(os.path.join(palace, "chroma.sqlite3"))
    except UnicodeEncodeError:
        pathname2url_raises = True
    if pathname2url_raises:
