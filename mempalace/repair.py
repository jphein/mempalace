    ...
    try:
        ...
        for i in range(0, expected, batch_size):
            ...
            if all_embeddings is not None:
                # Embeddings survived extraction (#1367 fork port) — reuse
                # them instead of recomputing every vector through ONNX.
                upsert_kwargs["embeddings"] = all_embeddings[i : i + batch_size]
            temp_col.upsert(
                documents=batch_docs, ids=batch_ids, metadatas=batch_metas, **upsert_kwargs
            )
...
def _promote_temp_collection(
    ...
    ids, docs, metas, embs = _extract_drawers(temp_col, expected, batch_size)
    ...
    for i in range(0, len(ids), batch_size):
        upsert_kwargs = {}
        if embs is not None:
            # Embeddings survived extraction (#1367 fork port) — reuse them
            # instead of recomputing every vector through ONNX.
            upsert_kwargs["embeddings"] = embs[i : i + batch_size]
        new_col.upsert(
